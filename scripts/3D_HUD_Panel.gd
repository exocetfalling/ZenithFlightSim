...
#	# Dynamic sizing
#	get_node('FlightData.aircraft').position = \
#		ui_element_dynamic_pos(current_viewport_size, \
#		Vector2(1920, 1080), \
#		Vector2(254, 819), \
#		Vector2(600, 600))
#	get_node('FlightData.aircraft').scale = \
...
#		Vector2(600, 600))
	...
	get_node('HUD/HUD_Ladder').rotation_degrees = -FlightData.aircraft_roll
	get_node('HUD/HUD_Ladder').position.y = (FlightData.aircraft_pitch / 90 * 3200) * cos(deg2rad(FlightData.aircraft_roll))
	get_node('HUD/HUD_Ladder').position.x = get_node('HUD/HUD_Ladder').position.y * tan(deg2rad(FlightData.aircraft_roll))
	...
	get_node("HUD/FlightPathVector").position.y = display_HUD_scale * (FlightData.aircraft_alpha / 90 * 3200)
	get_node("HUD/FlightPathVector").position.x = -display_HUD_scale * (FlightData.aircraft_beta / 90 * 3200)
...
#	$HUD/FlightPathVector/EnergyCaret.position.y = 5 * display_accel_z
	$HUD/FlightPathVector/E_Bracket.position.y = 4 * (FlightData.aircraft_alpha - 5)
	
	if (FlightData.aircraft_flaps > 0):
