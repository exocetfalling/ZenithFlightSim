...
# Shape Properties
var shape_step_length_divs := 1: set = set_step_length_divs
var shape_step_width_divs := 1: set = set_step_width_divs
var shape_smoothness := 0.5: set = set_smoothness

# Material Properties that not handled in shader
var mat_shader_type : int: set = set_shader_type
var mat_custom_shader : Shader: set = set_custom_shader

# LOD Properties
var lod_lod0_distance := 50.0: set = set_lod0_distance
...
# Public variables
var curve : Curve3D
var widths := []: set = set_widths
var valid_flowmap := false
var debug_view := 0: set = set_debug_view
...
func _get_property_list() -> Array:
	...
	if _material.gdshader != null:
		var shader_params := RenderingServer.shader_get_param_list(_material.gdshader.get_rid())
...
func _init() -> void:
	...
	_debug_material.gdshader = load(DEBUG_SHADER.shader_path) as Shader
	...
	_material.gdshader = load(BUILTIN_SHADERS[mat_shader_type].shader_path) as Shader
...
func set_shader_type(type: int):
	...
	if mat_shader_type == SHADER_TYPES.CUSTOM:
		_material.gdshader = mat_custom_shader
	else:
		_material.gdshader = load(BUILTIN_SHADERS[mat_shader_type].shader_path)
		for texture in BUILTIN_SHADERS[mat_shader_type].texture_paths:
			_material.set_shader_parameter(texture.name, load(texture.path) as Texture2D)
...
func _generate_flowmap(flowmap_resolution : float) -> void:
	...
	image = await WaterHelperMethods.generate_collisionmap(image, mesh_instance, baking_raycast_distance, baking_raycast_layers, _steps, shape_step_length_divs, shape_step_width_divs, self).completed
	...
	false # noise_with_tiling.lock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	...
	for x in _uv2_sides:
		...
		noise_with_tiling.blend_rect(noise_texture.get_data(), Rect2(0.0, 0.0, slice_width, noise_texture.get_height()), Vector2(slice_width + float(x) * slice_width, slice_width + (noise_texture.get_width() / 2.0)))
	false # noise_with_tiling.unlock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
